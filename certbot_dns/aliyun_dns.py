...
class AliyunDns(object):
    ...
    def add_dns_record(self, domain_name_re: str, domain_name: str, record_type: str, value: str) -> Optional[str]:
        try:
            ...
            return response
        except (ClientException, ServerException) as result:
            logger.exception(f'add dns record failed, exception=[{result}]')
            raise
    ...
    def del_dns_record(self, record_id):
        try:
            ...
            logger.exception(f'del dns record failed, exception=[{result}]')
            raise
